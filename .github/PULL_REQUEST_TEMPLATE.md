...
- [ ] Make sure your code lints (`pixi run nextflow lint . -format` and `pixi run pre-commit run --all-files`). When running locally, you may find that you get warnings/error for files that are not necessary to lint. If you would like to, you can specifically exclude those files/folders (e.g. `pixi run nextflow lint . -format -exclude .pixi -exclude .nf-test`). Alternatively, you can specific which folders to lint (e.g. `pixi run nextflow lint . -format main.nf nextflow.config modules/ subworkflows/ workflows/`)
