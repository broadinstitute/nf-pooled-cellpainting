...
Enter the sequence that represents barcoding reads of an empty vector""",
    ...
    def callonebarcode(
        ...
        master_cycles = []
        # Get actual cycle numbers from measurementdict keys instead of assuming 1-indexed range
        actual_cycles = sorted(measurementdict.keys())
        num_actual_cycles = len(actual_cycles)

        # Determine actual object count from first available measurement
        first_cycle = actual_cycles[0] if actual_cycles else None
        if first_cycle and measurementdict[first_cycle]:
            first_measurement = list(measurementdict[first_cycle].keys())[0]
            actual_objectcount = len(
                measurements.get_current_measurement(object_name, first_measurement)
            )
        else:
            actual_objectcount = objectcount

        # Use actual number of cycles and objects found in measurements
        score_array = numpy.zeros((num_actual_cycles, actual_objectcount))

        for idx, eachcycle in enumerate(actual_cycles):
