...
# Default assumes well_site_layouts.json lives alongside this script (both under
# assets/stitchcrop/); override via env var if it's staged somewhere else.
well_site_layouts_json = os.getenv(
    "WELL_SITE_LAYOUTS_JSON",
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "well_site_layouts.json"),
)
...
# Well-site position layouts (im_per_well_dict / phenix_im_per_well_dict) now live in
# well_site_layouts.json (shared with bin/stitch.py) instead of being hardcoded here twice.
with open(well_site_layouts_json) as _well_site_layouts_file:
    _well_site_layouts = json.load(_well_site_layouts_file)
im_per_well_dict = _well_site_layouts["im_per_well_dict"]
phenix_im_per_well_dict = _well_site_layouts["phenix_im_per_well_dict"]
