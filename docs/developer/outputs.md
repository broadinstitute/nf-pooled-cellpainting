...
When using Nextflow's reporting features, additional outputs are generated:
...
```bash
nextflow run main.nf -with-report execution_report.html -with-timeline -with-trace
```
