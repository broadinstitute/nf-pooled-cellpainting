...
The pipeline organizes outputs by processing stage and data hierarchy (batch → plate → well → site). Most outputs follow a predictable naming convention that includes the plate, well, and site information.

**Key directories:**

- `results/images/` - Processed image files (illumination functions, corrected images, stitched images)
- `results/workspace/analysis/` - Final CSV files with measurements
- `results/workspace/qc_reports/` - Quality control visualizations
- `results/workspace/load_data_csv/` - CellProfiler input files (useful for debugging)
...
This is where the final, most valuable outputs live. The combined analysis merges Cell Painting phenotype data with barcoding genotype data, giving you a complete picture of each cell.
