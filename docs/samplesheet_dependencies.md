...
3.  **Metadata JSON construction (Groovy)**: Each CellProfiler module's Nextflow subworkflow builds a canonical metadata JSON object (one entry per image/channel, see [python_scripts.md](python_scripts.md)) and passes it to `generate_load_data_csv.py` via `--metadata-json`. `generate_load_data_csv.py` itself does **no** filename parsing - every field (`well`, `site`, `channel`, `cycle`, `column_prefix`, etc.) is stated explicitly in that JSON.
    - **Illumination Calculation/Application**: `channel` comes directly from the samplesheet's `channels` column for both arms (via `expandImageChannels` in Groovy). `cycle` is carried through for barcoding, but explicitly nulled out for painting (`expandImageChannels(..., record_cycle: false)`) - phenotyping rounds use distinct channel names instead of `Cycle{NN}_`-prefixed columns, so real per-round `cycle` values never reach these CSVs.
